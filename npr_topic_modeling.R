...
plot(aggregate(posterior(topics)$topics[,3]~format.Date(as.Date(word(unlist(pubdatez[1:legit_articles]),2,4),"%d %b %Y"), "%U"),FUN=function(x){return(length(which(x >= .5)))}),type="l")
...
granger.test(candidate_df[,c(3,7)],1)
